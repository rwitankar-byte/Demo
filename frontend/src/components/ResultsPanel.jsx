...
import { Loader2, CheckCircle, AlertCircle, Info, Leaf, Share2 } from 'lucide-react';

const ResultsPanel = ({ result, isAnalyzing, analysisStep, language = 'en' }) => {
  // ─── Empty state ───
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[500px]">
        <div className="text-center space-y-4" data-testid="empty-state">
          ...
          <div>
            <h3 className="text-2xl font-bold mb-2" style={{ color: 'var(--gray-900)' }}>Ready to Analyze</h3>
            <p className="text-gray-600">Upload a leaf image to get started with disease detection</p>
          </div>
        </div>
      </div>
    );
  ...
  if (isAnalyzing) {
    const steps = [
      ...
      { id: 2, label: 'Detecting Disease' },
      { id: 3, label: 'Generating Advisory' }
    ];
      ...
      <div className="flex items-center justify-center h-full min-h-[500px]">
        <div className="w-full max-w-md space-y-6" data-testid="progress-indicator">
          <div className="text-center mb-8">
            <h3 className="text-2xl font-bold" style={{ color: 'var(--gray-900)' }}>Analyzing Your Crop</h3>
          ...
          {steps.map((step) => {
            ...
            return (
              <div key={step.id} className="flex items-center gap-4" data-testid={`progress-step-${step.id}`}>
                ...
                <div className={`font-bold ${isActive ? 'text-gray-900' : 'text-gray-400'}`}>
                  {step.label}
                </div>
              </div>
            );
  ...
  const handleWhatsAppShare = () => {
    const msg = [
      `*CropSense AI - Diagnosis Report*`,
      ``,
      `Disease: ${result.classification.label}`,
      `Severity: ${result.advisory?.severity || 'Unknown'}`,
      `Confidence: ${(result.classification.confidence * 100).toFixed(1)}%`,
      ``,
      `*Symptoms:* ${result.advisory?.visible_symptoms || 'N/A'}`,
      `*Cause:* ${result.advisory?.likely_cause || 'N/A'}`,
      `*Treatment:* ${result.advisory?.treatment || 'N/A'}`,
      ``,
      `*Farmer Advisory:* ${result.advisory?.plain_language_advisory || 'N/A'}`,
      ``,
      `Analyzed via CropSense AI`
    ].join('\n');
      ...
      <div className="bg-white rounded-2xl p-8 shadow-lg border" style={{ borderColor: 'var(--green-light)' }}>
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <span
                ...
                style={{ backgroundColor: severityColors.bg, color: severityColors.text, borderColor: severityColors.border }}
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-bold text-white transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5"
        ...
        <div className="mb-6">
          ...
          <div className="w-full h-3 rounded-full" style={{ backgroundColor: 'var(--green-pale)' }}>
            <div
              ...
              style={{ width: `${result.classification.confidence * 100}%`, backgroundColor: 'var(--green-mid)' }}
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-6">
          <div className="p-4 rounded-xl border" style={{ backgroundColor: 'var(--green-pale)', borderColor: 'var(--green-light)' }}>
            ...
            <p className="text-sm text-gray-700 whitespace-pre-line" data-testid="symptoms">
          ...
          <div className="p-4 rounded-xl border" style={{ backgroundColor: 'var(--green-pale)', borderColor: 'var(--green-light)' }}>
            ...
            <p className="text-sm text-gray-700 whitespace-pre-line" data-testid="cause">
          ...
          <div className="p-4 rounded-xl border" style={{ backgroundColor: 'var(--green-pale)', borderColor: 'var(--green-light)' }}>
            ...
            <p className="text-sm text-gray-700 whitespace-pre-line" data-testid="treatment">
          ...
          <div className="p-4 rounded-xl border" style={{ backgroundColor: 'var(--green-pale)', borderColor: 'var(--green-light)' }}>
            ...
            <p className="text-sm text-gray-700 whitespace-pre-line" data-testid="prevention">
        ...
        <div className="p-6 rounded-xl" style={{ backgroundColor: 'var(--teal-light)' }}>
          ...
          <p className="text-gray-700 leading-relaxed whitespace-pre-line" data-testid="advisory">
